...
The recommended way to set up your Matrix server is using the interactive setup script.
...
# This will generate a .env file with secure, random secrets
# and automatically detect your server's public IP address.
./setup.sh

# 3. Create the external Docker network (if it doesn't exist)
# The default network name is 'matrix-network'
...
After running `setup.sh`, a `.env` file is created. You should review this file before proceeding.
...
All configuration is handled via the `.env` file, which is generated by the `setup.sh` script.

The script will prompt you for your domains and then automatically generate all necessary secrets and detect your server's IP address.

### Environment Variables

The `setup.sh` script will generate the following variables in your `.env` file:

- `MATRIX_DOMAIN`: Your Matrix server domain.
- `ELEMENT_DOMAIN`: Your Element web client domain.
- `POSTGRES_USER`: Database username.
- `POSTGRES_PASSWORD`: A securely generated database password.
- `POSTGRES_DB`: Database name.
- `REGISTRATION_SHARED_SECRET`: A securely generated secret for registration tokens.
- `TURN_SHARED_SECRET`: A securely generated secret for the TURN server.
- `EXTERNAL_IP`: Your server's public IP, auto-detected for voice/video calls.
- `LIVEKIT_KEY`: Securely generated key for Element Call.
- `LIVEKIT_SECRET`: Securely generated secret for Element Call.
...
Your Matrix server is configured for voice and video calls using a **Coturn** TURN server.

### How it Works

- The `setup.sh` script automatically detects your server's public IP address and configures Coturn.
- If the script cannot detect the IP, it will prompt you to enter it manually.
- This IP is stored in the `EXTERNAL_IP` variable in your `.env` file.

### Firewall Requirements

For voice and video calls to work, you must open the following ports on your server's firewall:

- `3478/tcp` and `3478/udp` (For the TURN server)
- `49152-49172/udp` (For RTP media streams)

### Testing

Once deployed, you can test calls by:
1. Creating two users on your server.
2. Starting a voice or video call between them in Element.
